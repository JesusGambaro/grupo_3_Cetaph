...
const Detail = () => {
    ...
    useEffect(() => {
        (async () => {
            setLoading(true);
            try {
                const {data} = await axios.get(`http://localhost:9000/api/v1/album/${id}`);
                console.log(data);
                setDisk(data);
            } catch (e) {
                console.log(e);
    ...
    const handleSongChange = (dir) => {
        const index = disk.singles.findIndex(
                ...
                <div className="Detail">
                    <div className="album-info">
                        <div className="ImagenDisco">
                            <img
                                className="DiskImg"
                                src={disk.imagenes[currentImage].url}
                                alt=""
                            />
                        ...
                        <div className="DiscoInfo">
                            <h1 className="ArtistName">{disk?.artista[0]?.nombre}</h1>
                            <span>
                <label>Nombre:</label>
                <h1>{disk?.nombre}</h1>
              </span>
